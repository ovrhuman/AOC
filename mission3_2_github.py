...
def takedirection(d,who):
    path = []
    if d[0] == 'U':
        for i in range(1,int(d[1:])+1):
            who[1] += 1
            path.append(who.copy())
    elif d[0] == 'D':
        for i in range(1,int(d[1:])+1):
            who[1] -= 1
            path.append(who.copy())
...
def drawpath(route, who):
    path = []
    for d in route:
        path += takedirection(d, who)
    return path


def route_pedometer(route, intercepts, who):
    pedometer = 0 
    intercept_steps = []
    for d in route:
        if d[0] == 'U':
            for i in range(1,int(d[1:])+1):
                ...
                coord_ped = []
                pedometer += 1
                if who in intercepts:
                    coord_ped += who.copy()
                    coord_ped += [pedometer]
                    intercept_steps.append(coord_ped)
        elif d[0] == 'D':
            for i in range(1,int(d[1:])+1):
                who[1] -= 1
                coord_ped = []
...
stepsX = route_pedometer(routeX, newintercepts, positionX)
stepsY = route_pedometer(routeY, newintercepts, positionY)
...
for x in stepsX:
    Xstepsonly.append(x[2])
    Xinterceptsonly.append([x[0],x[1]])
    
for y in stepsY:
    Ystepsonly.append(y[2])
    Yinterceptsonly.append([y[0],y[1]])
    
intercept_totalstep = []    
totalsteponly = []
    
for i in range(len(Xinterceptsonly)):
    current_indextotalstep = []
    Yindex = Yinterceptsonly.index(Xinterceptsonly[i])
    totalstep = Ystepsonly[Yindex] + Xstepsonly[i]
    intercept_totalstep.append([Xinterceptsonly[i], totalstep])
    totalsteponly.append(totalstep)
    
print(intercept_totalstep[totalsteponly.index(min(totalsteponly))])
