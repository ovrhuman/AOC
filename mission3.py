# -*- coding: utf-8 -*-
"""
Created on Sat Oct 24 20:05:52 2020
...
def takedirection(d,who):
    ...
    if d[0] == 'U':
        ...
        for i in range(1,num+1):
            who[1] += 1
            path.append(tuple(who.copy()))   #use tuples so can be converted to
    elif d[0] == 'D':                        #a set later..
        num = int(d[1:])
        for i in range(1,num+1):
            who[1] -= 1
            path.append(tuple(who.copy()))
    elif d[0] == 'L':
        num = int(d[1:])
        for i in range(1,num+1):
            who[0] -= 1
...
#draw both wire paths
t1 = time.perf_counter()
Xpath = drawpath(routeX, positionX)
Ypath = drawpath(routeY, positionY)
t2 = time.perf_counter()

#find the intersections by creating union of the two sets
t3 = time.perf_counter()
XpathU = set(Xpath)
YpathU = set(Ypath)
inceptions = XpathU.union(YpathU)
t4 = time.perf_counter()

print('path drawing took {:.3f} seconds'.format(t2-t1))
print('finding intersections took {:.3f} seconds'.format(t4-t3))
